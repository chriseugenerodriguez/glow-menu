...
if ( !function_exists( 'tc_order_get_att' ) ) {

	function tc_order_get_att( $order, $att ) {
...
if ( !function_exists( 'tc_order_get_price_excluding_tax' ) ) {

	function tc_order_get_price_excluding_tax( $order, $item_id, $args = array() ) {
		$args = wp_parse_args( $args, array(
			'qty'   => '',
			'price' => '',
		) );
		...
		$product = $order->get_product_from_item( $order_items[ $item_id ] );
