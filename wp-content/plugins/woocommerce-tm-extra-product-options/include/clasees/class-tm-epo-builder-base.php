...
final class TM_EPO_BUILDER_base {
	...
	private function init_elements() {
		...
		$this->elements_array = array(
			...
			"textarea" => $this->add_element(
				"textarea",
				array( "required", "price", "sale_price", "text_before_price", "text_after_price", "price_type", "freechars", "hide_amount", "quantity", "placeholder", "min_chars", "max_chars", "default_value_multiple", "validation1" )
			),
			...
			"radiobuttons" => $this->add_element(
				"radiobuttons",
				array( "required", "text_before_price", "text_after_price", "hide_amount", "quantity", "use_url", "use_images", "use_lightbox", "swatchmode", "use_colors", "changes_product_image", "items_per_row", "clear_options", "options" )
			),

			"checkboxes" => $this->add_element(
				"checkboxes",
				array( "required", "text_before_price", "text_after_price", "hide_amount", "quantity", "limit_choices", "exactlimit_choices", "minimumlimit_choices", "use_images", "use_lightbox", "swatchmode", "use_colors", "changes_product_image", "items_per_row", "options" )
			),
		...
		if ( $this->woo_subscriptions_check ) {
			$this->elements_array["textarea"][24]['options'][] = array( "text" => __( "Subscription sing up fee", 'woocommerce-tm-extra-product-options' ), "value" => "subscriptionfee" );
			$this->elements_array["textfield"][24]['options'][] = array( "text" => __( "Subscription sing up fee", 'woocommerce-tm-extra-product-options' ), "value" => "subscriptionfee" );
			$this->elements_array["date"][24]['options'][] = array( "text" => __( "Subscription sing up fee", 'woocommerce-tm-extra-product-options' ), "value" => "subscriptionfee" );
			$this->elements_array["time"][24]['options'][] = array( "text" => __( "Subscription sing up fee", 'woocommerce-tm-extra-product-options' ), "value" => "subscriptionfee" );
		}
	...
	public final function add_setting_price_type3( $name = "" ) {
		return array(
			...
			"options"     => array(
				...
				array( "text" => __( 'Fee', 'woocommerce-tm-extra-product-options' ), "value" => "fee" ),
				array( "text" => __( 'Subscription sing up fee', 'woocommerce-tm-extra-product-options' ), "value" => "subscriptionfee" ),
			),
			"label"       => __( 'Price type', 'woocommerce-tm-extra-product-options' ),
		);
	}
	...
	private function builder_sub_options( $options = array(), $name = "multiple_selectbox_options", $counter = NULL, $default_value = NULL ) {
		...
		if ( $this->woo_subscriptions_check && $name != "multiple_selectbox_options" ) {
			$o["price_type"]['options'][] = array( "text" => __( "Subscription sing up fee", 'woocommerce-tm-extra-product-options' ), "value" => "subscriptionfee" );
		}
		if ( $name != "multiple_selectbox_options" ) {
		...
		foreach ( $options["title"] as $ar => $el ) {
			...
			$out .= "<div class='tm-cell col-3 tm_cell_images'>" . 
					...
					TM_EPO_HTML()->tm_make_field( $o["color"], 0 ) . 
					"</div>";
