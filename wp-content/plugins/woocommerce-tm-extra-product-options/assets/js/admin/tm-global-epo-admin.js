	...
	$.tmEPOAdmin = {
					...
					if ( name.length == 1 ) {
						...
						if ( field.is( ".element-variations" ) ) {
							...
							$( global_variation_object[ "variations" ] ).each( function ( index, variation ) {
								...
								field_values.push( '<option value="' + $.tmEPOAdmin.tm_escape( variation.variation_id ) + '">' + $.tmEPOAdmin.tm_unescape( tm_title ) + '</option>' );
							} );
		...
		builder_section_item_onClick: function () {
			...
			var $_html = $.tmEPOAdmin.builder_floatbox_template( {
				...
				"uniqid": tm_epo_admin.i18n_element_uniqid + ":" + _s.find( ".tm-builder-sections-uniqid" ).val()
			} );
		...
		tm_upload: function ( e ) {
			var $this = $( "#temp_for_floatbox_insert" ),
				...
				tm_color_picker = $this.find( ".builder_element_wrap" ).find( ".sp-replacer" );
	...
	$( document ).ready( function () {
		...
		if ( tm_settings_wrap.length > 0 ) {
			...
			$( document ).on( 'click', '.tc-save-button', function ( e ) {
				...
				$.post( tm_epo_admin.ajax_url, form.serialize() + '&save=save&action=tm_save_settings', function ( response ) {
					if ( response ) {
						if ( response.error == 1 ) {
