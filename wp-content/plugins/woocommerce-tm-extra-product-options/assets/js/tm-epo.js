(function ( $ ) {
	...
	var local_input_decimal_separator = (tm_epo_js.tm_epo_global_input_decimal_separator === "") ? tm_epo_js.currency_format_decimal_sep : getSystemDecimalSeparator(),
		...
		tc_epo_delay = window.tc_epo_delay || 500,
		tc_epo_animation_delay = window.tc_epo_animation_delay || 500,
	...
	function tm_init_epo( main_product, is_quickview, product_id, epo_id ) {
		...
		function tm_apply_validation( form ) {
			...
			if ( tm_epo_js.tm_epo_global_enable_validation == "yes" ) {
				...
				form.tc_validate( {
					focusInvalid: false,
					ignore: qty_selector + ",input.tm-qty:hidden[type='number'],input.input-text.qty,.ignore,.variations select,.tm-extra-product-options-variations input,.tm-extra-product-options-variations select",
		...
		function goto_error_item( item ) {
			...
			if ( el ) {
				...
				if ( elsection.is( '.section_popup' ) ) {
					...
					elsection.find( '.owl-carousel' ).trigger( 'to.owl.carousel', [ pos, 100 ] );
					setTimeout( function () {
			...
			if ( ! rules || ! $cart ) {
				...
				if ( (rules[ current_variation ] && current_variation !== 0) || rules[ 0 ] ) {
					...
					$( rules[ current_variation ] ).each( function ( id, rule ) {
						...
						if ( min <= qty && qty <= max ) {
							var _dc = parseInt( tm_epo_js.currency_format_num_decimals );
							switch ( type ) {
								case "percentage":
									price = price - (Math.ceil( (price * (value / 100)) * Math.pow( 10, _dc ) - 0.5 ) * Math.pow( 10, - _dc ));
		...
		function tm_epo_init( $cart_container, $composite_cart ) {
			...
			$epo_holder.find( '.tm-quantity' )
				...
				.on( 'showhide.cpfcustom', function ( event ) {
					...
					if ( ! field.is( '.tm-epo-variation-element' ) ) {
						...
						setTimeout( function () {
							quantity_selector.closest( '.owl-carousel' ).trigger( 'refresh.owl.carousel' );
						}, 200 );
					}
				} );
					...
					if ( show_total && qty > 0 ) {
						...
						if ( formatted_final_total && product_total_price >= 0 ) {
							...
							if ( (tm_epo_final_total_box == 'disable_change' || tm_epo_js.tm_epo_change_original_price == 'yes') ) {
								...
								if ( bundleid && $composite_cart && $cart.find( '.cpf-bto-price' ).length > 0 ) {
			...
			main_product.find( '.tm-variation-ul-color .tmhexcolorimage-li-nowh,.tm-variation-ul-image .tmhexcolorimage-li-nowh,ul.use_images_container .tmhexcolorimage-li-nowh,ul.use_colors_container .tmhexcolorimage-li-nowh' ).each( function ( i, el ) {
				el = $( el );
						...
						switch ( gallery.type ) {
							...
							case "woocommerce":
								...
								window.setTimeout( function () {
									gallery.element.trigger( 'woocommerce_gallery_init_zoom' );
									_window.trigger( 'resize' );
								}, 10 );
								break;
						}
		...
		function tm_custom_variations( form, item_id, $main_product, $epo_holder ) {
			...
			if ( $epo_holder.find( '.tm-epo-variation-element' ).length || $epo_holder.data( 'tm-epo-variation-element' ) ) {
				...
				uls.each( function ( i, el ) {
					el = $( el );
					var w = el.width() * 0.9, im = el.find( ".tmhexcolorimage" );
					im.css( { "min-width": w + "px", "min-height": w + "px" } );
				} );
			}
		}
			...
			if ( theme_name ) {
				...
				switch ( theme_name ) {
					...
					case 'handmade child theme':
						$( '.tm-owl-slider.owl-carousel' ).addClass( 'manual' );
						break;
	...
	_document.ready( function () {
		...
		$.ajaxPrefilter( function ( options, originalOptions, jqXHR ) {
			...
			if ( originalOptions.data && originalOptions.url ) {
				var _data = originalOptions.data;
				if ( typeof originalOptions.data == "string" ) {
					_data = originalOptions.data.tmparseParams();
				}
				var _urldata = originalOptions.url.slice( originalOptions.url.indexOf( '?' ) + 1 ).tmparseParams();
			...
			for ( var key in qv_container ) {
				if ( qv_container.hasOwnProperty( key ) ) {
					if ( qv_container[ key ].length ) {
						...
						if ( key == 'jckqv_quick_view' || key == "yith_quick_view_plugin" || key == "theme_flatsome" ) {
